...
class server(Thread):
    ...
    def __init__(
        ...
        self.PENDING_TRANSACTIONS_PATH = PENDING_TRANSACTIONS_PATH if custom_PENDING_TRANSACTIONS_PATH is None else custom_PENDING_TRANSACTIONS_PATH
    ...
    def get_transaction(self, data, node):
        ...
        logger.info(f"NODE:{self.host}:{self.port} -{custom_current_time}-{custom_sequence_number}-{custom_balance}")
        if GetTransaction(
            block, 
            the_transaction,
            custom_current_time=custom_current_time,
            ...
            server.send_transaction(the_transaction, except_client=node, custom_server=self)
            SaveBlock(
                block,
                custom_TEMP_BLOCK_PATH=self.TEMP_BLOCK_PATH,
                custom_TEMP_ACCOUNTS_PATH=self.TEMP_ACCOUNTS_PATH,
