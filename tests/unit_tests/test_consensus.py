...
class Test_Consensus(unittest.TestCase):
    ...
    def test_finished_main_save_from_no_part(self):
        ...
        self.assertEqual(
            expected_round_1_true_time, expected_true_time + block.round_2_time - 1
        )
    ...
    def test_candidate_blocks_check(self):
        ...
        CandidateBlock = candidate_block(
            [data_block for i in range(9)], [data_block_hash for i in range(8)]
        )
        CandidateBlock.candidate_blocks = [data_block for i in range(9)]
        CandidateBlock.candidate_blocks_hash = [data_block_hash for i in range(8)]
        unl_nodes = [i for i in range(10)]
        result = candidate_blocks_check(CandidateBlock, unl_nodes)
    ...
    def test_candidate_blocks_hashes_check(self):
        ...
        CandidateBlock = candidate_block(
            [data_block for i in range(8)], [data_block_hash for i in range(9)]
        )
        CandidateBlock.candidate_block_hashes = [data_block_hash for i in range(9)]
        CandidateBlock.candidate_blocks_hash = [data_block_hash for i in range(8)]
        unl_nodes = [i for i in range(10)]
        result = candidate_blocks_hashes_check(CandidateBlock, unl_nodes)
        self.assertIsNot(result, False)
    ...
    def test_round_process_round_1(self):
        ...
        self.assertEqual(
            result.validating_list[1].dump_json(), the_transaction.dump_json()
        )
    ...
    def test_process_candidate_blocks_hashes_false(self):
        ...
        CandidateBlock = candidate_block(
            [data_block for i in range(8)], [data_block_hash for i in range(8)]
        )
        the_new_list = []
        for i in range(8):
