...
def Check_Datas(block, transaction, custom_current_time=None, custom_balance=None, custom_sequence_number=None):
    ...
    balance = GetBalance(
        block, transaction.fromUser) if custom_balance is None else custom_balance
    ...
    get_sequance_number = GetSequanceNumber(
        transaction.fromUser) if custom_sequence_number is None else custom_sequence_number
    ...
    current_time = int(
        time.time()) if custom_current_time is None else custom_current_time
