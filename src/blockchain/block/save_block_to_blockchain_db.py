...
def SaveBlockstoBlockchainDB(
    ...
    if our_tx:
        ...
        with open(
            the_BLOCKS_PATH + str(block.sequance_number) + ".block", "wb"
        ) as block_file:
            pickle.dump(block, block_file, protocol=2)

        with open(
            the_BLOCKS_PATH + str(block.sequance_number) + ".accounts", "wb"
        ) as block_file:
            pickle.dump(
        ...
        with open(
            the_BLOCKS_PATH + str(block.sequance_number) + ".blockshash", "wb"
        ) as block_file:
            pickle.dump(
        ...
        with open(
            the_BLOCKS_PATH + str(block.sequance_number) + ".blockshashpart", "wb"
        ) as block_file:
            pickle.dump(
                GetBlockshash_part(
