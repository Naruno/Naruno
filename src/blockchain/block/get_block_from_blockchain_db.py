...
def GetBlockstoBlockchainDB(
    ...
    try:
        ...
        with open(
            the_BLOCKS_PATH + str(sequance_number) + ".block", "rb"
        ) as block_file:
            the_block = pickle.load(block_file)

        with open(
            the_BLOCKS_PATH + str(sequance_number) + ".accounts", "rb"
        ) as block_file:
            the_accounts = pickle.load(block_file)

        with open(
            the_BLOCKS_PATH + str(sequance_number) + ".blockshash", "rb"
        ) as block_file:
            the_blockshash = pickle.load(block_file)

        with open(
            the_BLOCKS_PATH + str(sequance_number) + ".blockshashpart", "rb"
        ) as block_file:
