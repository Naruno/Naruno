...
class mynode (Node):
    ...
    def send_my_block_hash(self, nodes):
        ...
        if system.raund_1 and not system.raund_2:
            ...
            for each_node in nodes:
                dprint(
                    "Raund 2: second ok of get candidate block hashes: " + str(each_node.__dict__))
                self.send_data_to_node(each_node, data)
    ...
    def get_candidate_block(self, data, node):
        ...
        if node_is_unl(node.id) and GetBlock().sequance_number == data["sequance_number"]:
            ...
            dprint("signatureverify: "+str(Ecdsa.verify("myblock"+merkle_root_of_signature_list,
                   Signature.fromBase64(data["signature"]), PublicKey.fromPem(node.id))))
    ...
    def send_full_chain(self, node=None):
        ...
        while SendData:
            ...
            data = {"fullblock": 1, "byte": (SendData.decode(encoding='iso-8859-1')), "signature": Ecdsa.sign(
                "fullblock"+str((SendData.decode(encoding='iso-8859-1'))), PrivateKey.fromPem(Wallet_Import(0, 1))).toBase64()}
            if not node is None:
            ...
            if not SendData:
                data = {"fullblock": 1, "byte": "end", "signature": Ecdsa.sign(
                    "fullblock"+"end", PrivateKey.fromPem(Wallet_Import(0, 1))).toBase64()}
                if not node is None:
                    self.send_data_to_node(node, data)
                else:
                    self.send_data_to_nodes(data)
    ...
    def send_full_accounts(self, node=None):
        ...
        while SendData:
            ...
            data = {"fullaccounts": 1, "byte": (SendData.decode(encoding='iso-8859-1')), "signature": Ecdsa.sign(
                "fullaccounts"+str((SendData.decode(encoding='iso-8859-1'))), PrivateKey.fromPem(Wallet_Import(0, 1))).toBase64()}
            if not node is None:
                self.send_data_to_node(node, data)
            else:
                self.send_data_to_nodes(data)
            ...
            if not SendData:
                data = {"fullaccounts": 1, "byte": "end", "signature": Ecdsa.sign(
                    "fullaccounts"+"end", PrivateKey.fromPem(Wallet_Import(0, 1))).toBase64()}
                if not node is None:
                    self.send_data_to_node(node, data)
                else:
                    self.send_data_to_nodes(data)
    ...
    def send_full_blockshash(self, node=None):
        ...
        while SendData:
            ...
            data = {"fullblockshash": 1, "byte": (SendData.decode(encoding='iso-8859-1')), "signature": Ecdsa.sign(
                "fullblockshash"+str((SendData.decode(encoding='iso-8859-1'))), PrivateKey.fromPem(Wallet_Import(0, 1))).toBase64()}
            if not node is None:
                self.send_data_to_node(node, data)
            else:
                self.send_data_to_nodes(data)
            ...
            if not SendData:
                data = {"fullblockshash": 1, "byte": "end", "signature": Ecdsa.sign(
                    "fullblockshash"+"end", PrivateKey.fromPem(Wallet_Import(0, 1))).toBase64()}
                if not node is None:
                    self.send_data_to_node(node, data)
                else:
                    self.send_data_to_nodes(data)
    ...
    def get_transaction(self, data, node):
        ...
        SendTransactiontoTheBlock(system, sequance_number=data["sequance_number"], signature=data["signature"], fromUser=data["fromUser"], toUser=data["to_user"],
                                  data=data["data"], amount=data["amount"], transaction_fee=data["transaction_fee"], transaction_sender=node, transaction_time=data["transaction_time"])
