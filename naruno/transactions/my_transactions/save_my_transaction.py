    ...
    if type(transaction_list) is list:
        ...
        for tx in transaction_list:
            ...
            entry_name_list.append(
                os.path.join(MY_TRANSACTION_PATH, sha256(name).hexdigest())
            )
            ...
            for entry in os.scandir(MY_TRANSACTION_PATH):
                if (
                    ...
                    if entry.name not in str(entry_name_list):
                        ...
                        if os.path.exists(
                            os.path.join(MY_TRANSACTION_PATH, "validated" + entry.name)
                        ):
                            os.remove(
                                os.path.join(
        ...
        for tx in transaction_list:
            ...
            with open(
                os.path.join(MY_TRANSACTION_PATH, sha256(name).hexdigest()), "w"
            ) as my_transaction_file:
                json.dump(transaction_list[tx]["tx"], my_transaction_file)
            if transaction_list[tx]["validated"]:
                with open(
                    os.path.join(
                        MY_TRANSACTION_PATH, "validated" + sha256(name).hexdigest()
                    ),
                    "w",
                ) as my_transaction_file:
            ...
            if transaction_list[tx]["sended"]:
                with open(
                    os.path.join(
                        MY_TRANSACTION_PATH, "sended" + sha256(name).hexdigest()
                    ),
                    "w",
                ) as my_transaction_file:
                    my_transaction_file.write("1")
        ...
        with open(
            os.path.join(
                MY_TRANSACTION_PATH, sha256(transaction_list[0].signature).hexdigest()
            ),
            "w",
        ) as my_transaction_file:
        ...
        if transaction_list[1]:
            with open(
                os.path.join(
                    MY_TRANSACTION_PATH,
                    "validated" + sha256(transaction_list[0].signature).hexdigest(),
                ),
                "w",
            ) as my_transaction_file:
                my_transaction_file.write("1")
        if transaction_list[2]:
            with open(
                os.path.join(
                    MY_TRANSACTION_PATH,
                    "sended" + sha256(transaction_list[0].signature).hexdigest(),
                ),
                "w",
            ) as my_transaction_file:
                my_transaction_file.write("1")
