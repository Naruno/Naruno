...
def SaveMyTransaction(transaction_list, clear=False):
    ...
    if type(transaction_list) is list:
        ...
        for tx in transaction_list:
            ...
            entry_name_list.append(os.path.join(MY_TRANSACTION_PATH, sha256(name).hexdigest()))
                ...
                if entry.name != "README.md" and not entry.name.startswith("validated") and not entry.name.startswith("sended"):
                    if entry.name not in str(entry_name_list):
                        ...
                        if os.path.exists(os.path.join(MY_TRANSACTION_PATH, "validated" + entry.name)):
                            os.remove(os.path.join(MY_TRANSACTION_PATH, "validated" + entry.name))
                        if os.path.exists(os.path.join(MY_TRANSACTION_PATH, "sended" + entry.name)):
                            os.remove(os.path.join(MY_TRANSACTION_PATH, "sended" + entry.name))
        ...
        for tx in transaction_list:
            ...
            with open(os.path.join(MY_TRANSACTION_PATH, sha256(name).hexdigest()), "w") as my_transaction_file:
                json.dump(transaction_list[tx]["tx"], my_transaction_file)
            if transaction_list[tx]["validated"]:
                with open(os.path.join(MY_TRANSACTION_PATH, "validated" + sha256(name).hexdigest()), "w") as my_transaction_file:
                    my_transaction_file.write("1")


            if transaction_list[tx]["sended"]:
                with open(os.path.join(MY_TRANSACTION_PATH, "sended" + sha256(name).hexdigest()), "w") as my_transaction_file:
                    my_transaction_file.write("1")
    elif type(transaction_list) is dict and transaction_list != {}:    
            with open(os.path.join(MY_TRANSACTION_PATH, sha256(transaction_list[0].signature).hexdigest()), "w") as my_transaction_file:
                json.dump(transaction_list[0].dump_json(), my_transaction_file)
            
            if transaction_list[1]:
                with open(os.path.join(MY_TRANSACTION_PATH, "validated" + sha256(transaction_list[0].signature).hexdigest()), "w") as my_transaction_file:
                    my_transaction_file.write("1")
            if transaction_list[2]:
                with open(os.path.join(MY_TRANSACTION_PATH, "sended" + sha256(transaction_list[0].signature).hexdigest()), "w") as my_transaction_file:
                    my_transaction_file.write("1")
