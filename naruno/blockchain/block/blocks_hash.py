...
def SaveBlockshash(the_blockshash, custom_TEMP_BLOCKSHASH_PATH=None):
    ...
    blockshash_db.set(
        "blockshash",
        the_blockshash) if custom_TEMP_BLOCKSHASH_PATH is None else KOT(
            "blockshash" + custom_TEMP_BLOCKSHASH_PATH,
            folder=get_config()["main_folder"] + "/db",
        ).set("blockshash", the_blockshash)
...
def SaveBlockshash_part(the_blockshash, custom_TEMP_BLOCKSHASH_PART_PATH=None):
    ...
    blockshash_db.set(
        "blockshash_part",
        the_blockshash) if custom_TEMP_BLOCKSHASH_PART_PATH is None else KOT(
            "blockshash" + custom_TEMP_BLOCKSHASH_PART_PATH,
            folder=get_config()["main_folder"] + "/db",
        ).set("blockshash_part", the_blockshash)
...
def GetBlockshash_part(custom_TEMP_BLOCKSHASH_PART_PATH=None):
    ...
    """
    record = (blockshash_db.get("blockshash_part")
