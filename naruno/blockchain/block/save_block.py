...
def SaveBlock(
    ...
    if not dont_clean:
        if not block.round_1 and not block.round_2:
            ...
            block.validating_list = sorted(
                block.validating_list, key=lambda x: x.fromUser
            )
    ...
    if delete_old_validating_list:
        ...
        for file in os.listdir("db/"):
            if ("db/" + file).startswith(the_TEMP_BLOCK_PATH) and not (
                ...
                if (
                    number == block.sequence_number + block.empty_block_number
    ...
    if not just_save_normal:
        ...
        block_db.set(
            highest_the_TEMP_BLOCK_PATH, block, custom_key_location=block_db_path_second
        )
