...
def process_candidate_blocks_hashes(block: Block,
    ...
    for candidate_block_hash in candidate_class.candidate_block_hashes[:]:
        logger.debug(
            f"Candidate block hash previous_hash {candidate_block_hash}")

        tx_valid = 1

        for other_block in candidate_class.candidate_block_hashes[:]:
            if (candidate_block_hash != other_block
                    and candidate_block_hash["previous_hash"]
                    == other_block["previous_hash"]):
                tx_valid += 1
        logger.debug(
            f"Hash valid of previous_hash  {candidate_block_hash} : {tx_valid}"
        )
        if tx_valid > (((len(unl_nodes) + 1) * 80) / 100):
            logger.info(
    ...
    if current_hash != {
        ...
        if current_hash["signature"] == "self":
            for other in candidate_class.candidate_block_hashes[:]:
                if current_hash != other and current_hash["hash"] == other[
                        "hash"]:
                    current_hash = other
                    break
        ...
        if previous_hash["signature"] == "self":
            for other in candidate_class.candidate_block_hashes[:]:
                if (previous_hash != other and previous_hash["previous_hash"]
                        == other["previous_hash"]):
                    previous_hash = other
                    break
