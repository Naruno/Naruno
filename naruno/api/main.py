...
from naruno.lib.settings_system import d_mode_settings, t_mode_settings, the_settings
...
def print_wallets_page():
    logger.debug(f"{request.remote_addr} {request.method} {request.url} {request.data}")
...
def sequence_get_page():
    ...
    return jsonify(
        GetSequanceNumber(address, account_list=custom_account_list, dont_convert=True)
    )
...
def start(host=None, port=None, test=False):
    ...
    result = (
        ...
        if test is False
        else create_server(app, host=host, port=args.port)
    )
    return result
