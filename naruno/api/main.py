...
def send_coin_data_page():
    ...
    try:
        ...
        sequence_number = (
            str(request.form["sequence_number"])
            if "sequence_number" in request.form
            else None
        )
        block = None
...
def block_get_page_proccess(the_server):
    ...
    trigger = perpetualTimer(
        the_block.consensus_timer, the_consensus_trigger, the_consensus=True
    )
