...
class Decentra_Network_Docker:
    ...
    def run(self):
        time.sleep(5 * self.number_of_nodes)
        os.system(
            "docker run -v decentra-network-db:/Decentra-Network/src/db/ -v decentra-network-logs:/Decentra-Network/src/logs/ --network dn-net -p 8000:8000 -p 7999:7999 -dit decentra-network-api"
        )
        for i in range(self.number_of_nodes):
            os.system(
                f"docker run -v decentra-network-db-{i}:/Decentra-Network/src/db/ -v decentra-network-logs-{i}:/Decentra-Network/src/logs/ --network dn-net -p {8100 + i + 1}:8000 -p {8010 + i + 1}:{8010 + i + 1} -dit {i}"
            )
