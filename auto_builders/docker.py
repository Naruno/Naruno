...
class Decentra_Network_Docker:
    ...
    def run(self):
        ...
        for i in range(self.number_of_nodes):
            os.system(f"docker run -v decentra-network-{i}:/Decentra-Network/src/db/ --network dn-net -p {8100 + i + 1}:8000 -p {8010 + i + 1}:{8010 + i + 1} -dit {i}")
    ...
    def creating_the_wallets(self):
        ...
        for i in range(self.number_of_nodes):
            create_wallet_2 = json.loads(urllib.request.urlopen(f"http://localhost:{8100 + i + 1}/wallet/create/123").read().decode())
    ...
    def starting_the_nodest(self):
        ...
        for i in range(self.number_of_nodes):
            node_start_2 = json.loads(urllib.request.urlopen(f"http://localhost:{8100 + i + 1}/node/start/172.19.0.{i+3}/{8010 + i + 1}").read().decode())

    def unl_nodes_settting(self):

        node_id_1 = json.loads(urllib.request.urlopen("http://localhost:8000/node/id").read().decode())
        for i in range(self.number_of_nodes):
            urllib.request.urlopen(f"http://localhost:{8100 + i + 1}/node/newunl/?{node_id_1}")
        
        if self.number_of_security_circle == 1:
            for i in range(self.number_of_nodes):
                node_id_2 = json.loads(urllib.request.urlopen(f"http://localhost:{8100 + i + 1}/node/id").read().decode())
                urllib.request.urlopen(f"http://localhost:8000/node/newunl/?{node_id_2}")
                for i_n in range(self.number_of_nodes):
                    if not i == i_n:
                        urllib.request.urlopen(f"http://localhost:{8100 + i_n + 1}/node/newunl/?{node_id_2}")
        else:
    ...
    def connecting_the_nodes(self):
        ...
        for i in range(self.number_of_nodes):
            urllib.request.urlopen(f"http://localhost:8000/node/connect/172.19.0.{i+3}/{8010 + i + 1}")
        
        if self.number_of_security_circle == 1:
            for i in range(self.number_of_nodes):
                for i_n in range(self.number_of_nodes):
                    if not i == i_n:
                        urllib.request.urlopen(f"http://localhost:{8100 + i + 1}/node/connect/172.19.0.{i_n+3}/80{i_n+1}0")
                        time.sleep(1)
        else:
            nodes_list = list(range(self.number_of_nodes))
            circle_list = [nodes_list[x:x+((self.number_of_nodes+1)//self.number_of_security_circle)] for x in range(0, len(nodes_list), ((self.number_of_nodes+1)//self.number_of_security_circle))]
