...
class Decentra_Network_Local:
    ...
    def run(self):
        ...
        for i in range(self.number_of_nodes):
            print(
                f"nohup python3 Decentra-Network-{i+1}/src/api.py -p 80{i+1}0 &")
            os.system(
                f"nohup python3 Decentra-Network-{i+1}/src/api.py -p 80{i+1}0 &")

    def creating_the_wallets(self):

        create_wallet_1 = json.loads(urllib.request.urlopen(
            "http://localhost:8000/wallet/create/123").read().decode())
        for i in range(self.number_of_nodes):
            create_wallet_2 = json.loads(urllib.request.urlopen(
                f"http://localhost:80{i+1}0/wallet/create/123").read().decode())

    def starting_the_nodest(self):

        node_start_1 = json.loads(urllib.request.urlopen(
            "http://localhost:8000/node/start/0.0.0.0/7999").read().decode())
        for i in range(self.number_of_nodes):
            node_start_2 = json.loads(urllib.request.urlopen(
                f"http://localhost:80{i+1}0/node/start/0.0.0.0/800{i+1}").read().decode())

    def unl_nodes_settting(self):

        node_id_1 = json.loads(urllib.request.urlopen(
        ...
        if self.number_of_security_circle == 1:
            for i in range(self.number_of_nodes):
                node_id_2 = json.loads(urllib.request.urlopen(
            ...
            circle_list = [nodes_list[x:x+((self.number_of_nodes+1)//self.number_of_security_circle)]
                           for x in range(0, len(nodes_list), ((self.number_of_nodes+1)//self.number_of_security_circle))]
    ...
    def connecting_the_nodes(self):
        ...
        for i in range(self.number_of_nodes):
            urllib.request.urlopen(
                f"http://localhost:8000/node/connect/0.0.0.0/800{i+1}")
        ...
        if self.number_of_security_circle == 1:
            for i in range(self.number_of_nodes):
                for i_n in range(self.number_of_nodes):
                    if not i == i_n:
                        urllib.request.urlopen(
                            f"http://localhost:80{i+1}0/node/connect/localhost/800{i_n+1}")
                        time.sleep(1)
        else:
            nodes_list = list(range(self.number_of_nodes))
            circle_list = [nodes_list[x:x+((self.number_of_nodes+1)//self.number_of_security_circle)]
...
if __name__ == '__main__':
    ...
    if args.securitycirclenumber is not None:
        temp_environment = Decentra_Network_Local(
            args.nodenumber, args.securitycirclenumber)
    else:
